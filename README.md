...
для повышения безопасности морского судоходства. Сообщения АИС передаются с помощью радиоволн диапазона УКВ на ближайшие суда
и содержат, среди прочего, информацию об идентификации, местоположении, скорости и курсе передающих судов.
Таким образом, АИС служит инструментом для предотвращения столкновений и повышения ситуационной осведомленности.
...
наблюдение практически за любым крупным судном. Примером приложения, использующего данные АИС является сайт
...
В противном случае они могут быть признаком незаконной деятельности, такой как обмен контрабандой или наркотиками.  
**В данной работе мы рассмотрим поиск такой аномалии с помощью Apache Spark Structured Streaming.** 
Почему Apache Spark?  
...
К сожалению, в бесплатном доступе есть данные AIS, охватывающие только прибрежные территории, публикуемые либо энтузиастами либо
...
- BaseDateTime время отправки данных(время события) будет - текущее время минус 1-2 секунды, для того чтобы слишком большое кол-во записей не имело одинаковое время
...
- Все записи окна проверяем между собой с помощью формулы Гаверсинуса, если обнаружим, что есть точки, расстояние между которыми
